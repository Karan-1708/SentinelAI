...
class IncidentReportGenerator:
    ...
    def generate(
        ...
        """
        Generate a complete PDF report for an incident.

        Args:
            incident: dict with id, created_at, severity, threat_label,
                      confidence, anomaly_score, source_ip, dest_ip
            shap_data: dict with feature_contributions list
            mitre_data: list of technique dicts

        Returns:
            PDF as bytes (suitable for StreamingResponse)
        """
        ...
        story.append(Paragraph(
            f"<b>Incident ID:</b> {incident.get('id', 'N/A')} &nbsp;&nbsp; "
        ...
        table_data = [
            ["Field", "Value"],
            ["Severity", severity],
            ["Threat Classification", incident.get("threat_label", "N/A")],
            ["Confidence", f"{incident.get('confidence', 0):.1%}"],
            ["Anomaly Score", f"{incident.get('anomaly_score', 0):.4f}"],
            ["Source IP", incident.get("source_ip") or "N/A"],
            ["Destination IP", incident.get("dest_ip") or "N/A"],
        ]
        ...
        if mitre_data:
            ...
            for ttp in mitre_data:
                ...
                name = ttp.get("technique_name", tid)
                tactic = ttp.get("tactic", "unknown").replace("-", " ").title()
                url = ttp.get("url", "")
                story.append(Paragraph(
                    f"&bull; <b>{tid}</b> — {name} &nbsp;<i>({tactic})</i>"
                    + (f" <font color='blue'>[{url}]</font>" if url else ""),
                    bullet_style,
                ))
            story.append(Spacer(1, 0.5 * cm))
        ...
        if contributions:
            ...
            # Render SHAP bar chart using matplotlib, embed as image
        ...
        label = incident.get("threat_label", "BENIGN")
    ...
    def _render_shap_chart(self, contributions: list[dict]) -> Optional[bytes]:
        """Render SHAP values as a horizontal bar chart, return PNG bytes."""
        try:
            import matplotlib
            matplotlib.use("Agg")
            ...
            values = [c["shap_value"] for c in sorted_contribs]
            colors_list = ["#dc2626" if v > 0 else "#2563eb" for v in values]

            fig, ax = plt.subplots(figsize=(8, 5))
            bars = ax.barh(range(len(features)), values, color=colors_list, height=0.6)
            ...
            logger.warning("Failed to render SHAP chart: %s", e)
            return None
